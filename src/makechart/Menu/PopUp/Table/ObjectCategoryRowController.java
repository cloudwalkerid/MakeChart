 ...
 * @author ASUS
 */
public class ObjectCategoryRowController implements Initializable {
    ...
    private ObjectCategoryRowController parentController;

    private ArrayList<ObjectCategoryRowController> childController;
    ...
    public void setPane(Pane thisPane, ObjectCategoryRowController parentController
            ...
            if (objectCategory.getObjectCategorys() != null && objectCategory.getObjectCategorys().size() != 0) {
                for (ObjectCategory item : objectCategory.getObjectCategorys()) {
                    try {
                        ...
                        FXMLLoader loader = new FXMLLoader(getClass().getResource("ObjectCategoryRow.fxml"));
                        Pane newLoadedPane = loader.load();
                        newLoadedPane.setId("Baru");
                        ObjectCategoryRowController columnItemController = loader.<ObjectCategoryRowController>getController();
                        columnItemController.setObjectInformationColumn(newD);
                        vBoxCategory.setVisible(true);
                        vBoxCategory.getChildren().add(newLoadedPane);
    ...
    public void OnDrop(String content) {
        try {
            ...
            vBoxCategory.setVisible(true);
            vBoxCategory.getChildren().add(newLoadedPane);
            childController.add(rowItemController);
            //this.thisPane.setMinSize(Control.USE_COMPUTED_SIZE, Control.USE_COMPUTED_SIZE);
            this.thisPane.setPrefSize(Control.USE_COMPUTED_SIZE, Control.USE_COMPUTED_SIZE);
            rowItemController.setPane(newLoadedPane, this, this.addUpdateTableController, null);
        } catch (Exception ex) {
            ex.printStackTrace();
        }
    ...
    public void OpenCategoryMenu() {
        try {
            ...
            if(parentController==null){
                addController.setRowController(objectInformationColumnTable, objectCategory, this, true, addUpdateTableController);
            }else{
                addController.setRowController(objectInformationColumnTable, objectCategory, this, false, addUpdateTableController);
            }
    ...
    public ObjectCategory getObjectCategory(){
        objectCategory.setNameColumnHasil("r_"+objectCategory.getNameColumnHasil());
        if(childController.size()!=0){
            for(int i=0; i<childController.size(); i++){
                objectCategory.getObjectCategoryAll().add(childController.get(i).getObjectCategory());
            }
        }
